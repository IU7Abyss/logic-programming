- [Основа](#Основа)
    ...
    + [Что используется в Prolog?](#Что-используется-в-Prolog)
- [Механизм унификации](#Механизм-унификации)
    ...
    + [Как представляется резольвента в Prolog?](#Как-представляется-резольвента-в-Prolog)
    + [Что такое редукция?](#Что-такое-редукция)
    + [Во всей резольвенте применяется подстановка?](#Во-всей-резольвенте-применяется-подстановка)
    + [Как изменяется резольвента?](#Как-изменяется-резольвента)
    + [Как растет стек в процессе обработки резольвенты?](Как-растет-стек-в-процессе-обработки-резольвенты)
    + [Чем завершается унификация?](#Чем-завершается-унификация)
    + [Когда будет получен ответ и что будет дальше? (условие получения ответа на вопрос)](Когда-будет-получен-ответ-и-что-будет-дальше-условие-получения-ответа-на-вопрос)
    + [Что происходит после нахождения решения?](Что-происходит-после-нахождения-решения)
- [Механизм отката](#Механизм-отката)
    + [Что такое откат?](Что такое откат?)
    + [Как работает механизм отката](Как-работает-механизм-отката)
    + [Когда происходит откат?](Когда-происходит-откат)
    + [Что такое тупик?](Что-такое-тупик)
...
__Резольвента__ - набор (конъюнкция или дизъюнкция) целей (утверждений) истинность (выводимость) которых надо доказать.

### Как представляется резольвента в Prolog?
_Резольвента_ в _Prolog_ представляется в виде __стека__.

### Что такое редукция?
__Редукция__ - замена цели телом того правила, заголовок которого унифицируется с целью

### Во всей резольвенте применяется подстановка?
...
WTF? Стек в процессе обработки резольвенты растёт _влево_. (Прокатывает, в крайнем случае рисовать на бумаге)
...
### Когда будет получен ответ и что будет дальше? (условие получения ответа на вопрос)
Когда резольвента пуста, затем будет произведён откат и попытка найти другой ответ.
...
2. Отменяется _последняя_ выполненная _редукция_.
3. Выполняется _реконкретизация_ переменных, которые были конкретизированы на последнем шаге.
